...
(module tamale (lib "lispish.ss" "5L")
  ...
  ;;; VAL2.
  (define (interpolate-real fraction val1 val2)
    (+ val1 (* fraction (- val2 val1))))

  ;;; Return a value FRACTION percent of the distance between VAL1 and
  ;;; VAL2, rounded to an integer.
  (define (interpolate-int fraction val1 val2)
    (number->integer (interpolate-real fraction val1 val2)))
  ...
  (defmethod (interpolate-value (fraction <real>) 
                                (from <real>) (to <real>))
    (interpolate-real fraction from to))

  (defmethod (interpolate-value (fraction <real>)
                                (from <integer>) (to <integer>))
    (interpolate-int fraction from to))

  (defmethod (interpolate-value (fraction <real>)
                                (from <point>) (to <point>))
    (point (interpolate-value fraction (point-x from) (point-x to))
           (interpolate-value fraction (point-y from) (point-y to))))

  (defmethod (interpolate-value (fraction <real>)
                                (from <rect>) (to <rect>))
    (rect (interpolate-value fraction (rect-left from) (rect-left to))
          (interpolate-value fraction (rect-top from) (rect-top to))
          (interpolate-value fraction (rect-right from) (rect-right to))
          (interpolate-value fraction (rect-bottom from) (rect-bottom to))))

  (defmethod (interpolate-value (fraction <real>)
                                (from <color>) (to <color>))
    (color (interpolate-value fraction (color-red from) (color-red to))
           (interpolate-value fraction (color-green from) (color-green to))
           (interpolate-value fraction (color-blue from) (color-blue to))
           (interpolate-value fraction (color-alpha from) (color-alpha to))))
