...
(define special-eval
  (let ((orig-eval (current-eval)))
    (lambda (expr)
      ...
      (orig-eval
       (let loop ((expr (datum->syntax-object #'here expr)) (q 0))
         (syntax-case expr (: _)
        ...
        (let loop ((path path) (cur-path cur-path))
          (let ((x (regexp-match "^([^/]*/)(.*)" path)))
            (if (and x (>= (string-length cur-path) (string-length (cadr x)))
              ...
              (regexp-replace "(/?)([^/]*)$"
                              (regexp-replace "[^/]*/" cur-path "../")
                              (string-append "\\1" path)))))
        path))
    path))

(defsubst* (with-params ((param val) ...) body ...)
  (thunk (parameterize ((param val) ...) (output (begin body ...)))))
...
(define (process-metas
  ...
  (define-values (string-begin string-end)
    (cond ((list? string-quotes) (apply values string-quotes))
          ...
          (else (values "\"" "\""))))
  (define spaces-regexp   (regexp "^ *$"))
  (define nonspace-regexp (regexp "[^ ]"))
  ...
  (define (disp x)
    (let ((x (if scm? x (string-quote x))))
      (if (or (not split-lines?) scm?)
        ...
        (let ((p (cond ((regexp-match-positions nonspace-regexp x) => caar)
                       (else #f))))
     ...
     ((regexp-match-positions meta-regexp str) =>
      (lambda (x)
        (let ((prefix (substring str 0 (caar x)))
          ...
          (when (or scm?
                    (not split-lines?)
                    (not (regexp-match spaces-regexp prefix)))
            (disp prefix))
          (cond
           ((and (not scm?) token2
           ...
           ((equal? token meta-end) (close)
            (loop (if (and split-lines? (regexp-match spaces-regexp suffix))
...
(define* text-replacements
  (map (lambda (x)
         (list (regexp (car x)) (lambda (_ txt) (list (cadr x) txt))))
       `(("\\*([^*]*)\\*" b:) ("_([^_]*)_" u:) ("/([^/]*)/" i:))))
...
(define* (((form:->:: w:) . args1) . args2)
  (let-values (((args2 body) (keys/args args2)))
    (apply w: (append args2 args1 body)))) ; arg2 precede
(define* (((form~:->~:: w~:) x . args1) . args2)
  (let-values (((args2 body) (keys/args args2)))
    (apply w~: x (append args2 args1 body)))) ; arg2 precede

(defform* (recform: &keys (tag ::tag #f) (n ::n 1) &rest-keys body)
  (let-values (((keys body) (keys/args body)))
    (cond
     ((zero? n) body)
...
(defwrapper* options: (list~:: "select" ::subtag 'option~:)
  ::indent? #t)
(defform* (select-options: . keys+options)
  (let-values (((keys options) (keys/args keys+options)))
...
(define* html-specials
  '((#\space "nbsp") (#\C "copy") (#\R "reg") (#\T "trade") (#\* "bull")))
          ...
          (unless (eof-object? ch)
            (cond ((eq? ch #\nul)
                  ...
                  ((and (eq? ch #\\) (zero? literal))
                   (display-char (read-char) html-specials))
                  ((> literal 0) (display ch))
                  (else (display-char ch html-quotes)))
            (loop))))
       ...
       (output-to-html (if (symbol? name) (symbol->string name) name)
         (thunk
           (let ((contents
             ...
             (output
              (apply document:
                     ::comment1 '("Generated by html.scm, "
                                  "Eli Barzilay: http://www.barzilay.org/")
                     ::comment2 "Generated by html.scm"
                     contents))))))
