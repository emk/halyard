...
(define* (l-hash-table-get vec keys . thunk)
  (let ((len (length keys)))
    (let loop ((obj (vector-ref vec (min len l-hash-vector-length)))
...
(define* (l-hash-table-put! vec keys value)
  (let* ((len (length keys))
         (table (vector-ref vec (min len l-hash-vector-length))))
    (when (eq? table *nothing*)
      (set! table (if (zero? len) value (make-hash-table 'weak)))
      (vector-set! vec (min len l-hash-vector-length) table))
    (unless (zero? len)
      (let loop ((obj table)
                 (keys (if (< len l-hash-vector-length) keys (cons len keys))))
...
(define (sequential-generators gen . rest)
  ;; the following line crashes on an mzc byte-code bug
  ;; let-values (((new) #f) ((fst step done? value) (values . gen)))
  (let ((new #f)
        (fst (1st gen)) (step (2nd gen)) (done? (3rd gen)) (value (4th gen)))
    (define (next!)
      (and (pair? rest)
...
(define* *echo-write-handler* (make-parameter display))
