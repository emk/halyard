...
(define* (struct-type->class stype)
  (hash-table-get
   ...
   (thunk
     (let-values (((name fields accessor mutator super skipped?)
...
(add-method allocate-instance
  (make-method (list <class>)
    (named-lambda method:allocate-instance (call-next-method class)
...
(define (check-initargs class initargs)
  ...
  (let ((valid-initargs (%class-valid-initargs class)))
    (let loop ((args initargs))
      (cond ((or (null? args) (not valid-initargs)) #t)
...
(defsubst* (rec-make (name class arg ...) ...)
  (let ((name (allocate-instance class)) ...)
...
(let ((old-add-method add-method) (bad-name-regexp (regexp ":[0-9]*:[0-9]*$")))
            ...
            (unless (and n (not (eq? n '-anonymous-)))
              (%set-method-name!
               ...
               (let* ((psym (object-name (%method-procedure method)))
                 ...
                 (if (or (not pstr) (regexp-match bad-name-regexp pstr))
...
(define* <procedure>
  (make <procedure-class> :direct-supers (list <builtin> <function>)
                          :direct-slots  '()
                          :name          '<procedure>))
;;>> <primitive-procedure>
